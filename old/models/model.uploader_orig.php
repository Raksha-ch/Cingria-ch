...
//	 4) FILE INFO ARRAY
//	 			4.1) Save File Info Array
//	 			4.2) Flush File Info Array
//	 5) FILTER FILE TYPES
...
//  ******************************************************************************************************************************************	
	...
	const  MAX_FILE_SIZE  				= '8000000';
  	...
  	public $extension_r 				=  array( ".doc" , ".flv" , ".gif" , ".jpg" , ".pdf" , ".png" , ".psd" , ".swf" , ".tif" , ".zip" );
...
//  ******************************************************************************************************************************************  		
	...
	function setFileExtension( $_file_src ) 					{ $this->_file_ext = strtolower(strrchr($_file_src ,".")) ;}
 	function getFileExtension() 								{ return $this->_file_ext ;}

	function setNewFileName( $_file_src ) 						{ $this->_new_file_name = $_file_src ;}
	function getNewFileName() 									{ return $this->_new_file_name ;}
	
	function getFileName() 										{ return $this->_file_name ;}
	function setFileName( $_file_src ) 							{
																if ( strrpos( $_file_src , '.' ) ) { $this->_file_name = substr ($_file_src , 0 , strrpos( $_file_src , '.' ) ) ;}
																else { $this->_file_name = $_file_src ;}
																}
	
	function setFileParentId( $_file_parent_id ) 				{ $this->_file_parent_id = $_file_parent_id ;}
	function getFileParentId () 								{ return $this->_file_parent_id ;}
	...
	function setFileExtField( $_file_ext_field ) 				{ $this->_file_ext_field = $_file_ext_field ;}
  	function getFileExtField() 									{ return $this->_file_ext_field ;}

	function setFileNameField( $_file_name_field ) 				{ $this->_file_name_field = $_file_name_field ;}
  	function getFileNameField() 								{ return $this->_file_name_field ;}

	function setFileParentField( $_file_parent_field ) 			{ $this->_file_parent_field = $_file_parent_field ;}
  	function getFileParentField() 								{ return $this->_file_parent_field ;}

	function setFileCreationField( $_file_creation_field ) 		{ $this->_file_creation_field = $_file_creation_field ;}
	function getFileCreationField() 							{ return $this->_file_creation_field ;}
  	
	function setLanguageId( $_language_id ) 					{ $this->_language_id = $_language_id ;}
	function getLanguageId() 									{ return $this->_language_id ;}
	
	function setFileTitle( $_upload_file_title ) 				{ $this->_upload_file_title = $_upload_file_title ;}
	function getFileTitle() 									{ return $this->_upload_file_title ;}

	function setFileDescription( $_upload_file_description ) 	{ $this->_upload_file_description = $_upload_file_description ;}
	function getFileDescription() 								{ return $this->_upload_file_description ;}

	function setFileRank( $_upload_file_rank ) 					{ $this->_upload_file_rank = $_upload_file_rank ;}
	function getFileRank() 										{ return $this->_upload_file_rank ;}
	
	function setFileDisplay( $_upload_file_display ) 			{ $this->_upload_file_display = $_upload_file_display ;}
	function getFileDisplay() 								{ return $this->_upload_file_display ;}

	function setRedir($_redir) 									{ $this->_redir = $_redir; }
	function getRedir() 										{ return $this->_redir; }	

	function getFileNameAndExtension() 							{ return $this->_file_name.$this->_file_ext ;}
  	function getFileRelativeUrl() 								{ return $this->_target.$this->_file_name.$this->_file_ext ;}
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function uploadFile() {
		...
		try {
			...
			if ( !move_uploaded_file( $_FILES['file']['tmp_name'] , $this->getFileRelativeUrl() ) ) {
				...
				if ( $this->_error_flag == 0 ) {
					...
					if ( $this->crud->insert( $this->getTable() , $this->saveFileInfoArray() ) ) {
						
						$serialized_info=serialize( $this->uploadedFileInfo() );	
						header( "location: " . $this->getRedir() . '&info_box=uploaded&file=' . $this->getFileName() . $this->getFileExtension() . '&serialized_info=' . $serialized_info );	
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
  ...
  function editFile() {
			...
			try {
				...
				if ( $this->crud->update( $this->getTable() , $this->crud->getMatchingFields( $this->getTable() ) , $_POST['file_field'] , $_POST['file_id'] ) ) {
					
					$serialized_info = serialize( $this->editedFileInfo() );	
					header( "location: " . $this->getRedir() . '&info_box=edited&file=' . $this->getNormalizedFileName() . $this->getFileExtension() . '&serialized_info=' . $serialized_info );		
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
  	...
  	function renameFileOnServer() {
 		...
 		try {
			...
			if ( !rename( $this->getTarget() . $this->getNewFileName() . $this->getFileExtension(),
						  $this->getTarget() . normalize($this->getFileName()) . $this->getFileExtension() ) ) {
 		
				throw new myExceptions( 'Couldn\'t move <em>' . $this->getFileName() . '</em> file to server location <em>' . $this->getTarget() . '</em>.' );
 			}
	        ...
	        if( !in_array( $file, $ignore ) ) {
				...
				if ($only_dirs == true) {
					...
					if ( is_dir( "$path/$file" ) ) {
						...
						if ($return_type == 'return_options') {
							$this->returnOptions($path, $file, $spaces);
						}
					
						if ($return_type == 'return_links') {
							print $spaces.$file;
							//$this->returnLinks($path, $file, $spaces);
						}
						
						if ($return_type == 'return_list_items') {
...
function returnListItems($path, $file, $spaces) {
	...
	<a href="?page=uploader&action=delete_dir&dir_name='.$path.'/'.$file.'" onclick="return(confirm(\'Delete directory and its content?\'))" class="delete_hover">delete</a> | <a href="?page=uploader&action=rename_dir&dir_name='.$path.'/'.$file.'">rename</a></span></li>';
}
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function saveFileInfoArray() {
	
	   $now = date("Y-m-d H:i:s");
	
	   $this->_saved_array =
	   array( $this->getFileNameField()	   	=>$this->getFileName(),
			  $this->getFileExtField()	   	=>$this->getFileExtension(),
			  $this->getFileParentField()	=>$this->getFileParentId(),
			  $this->getFileCreationField()	=>$now,
			  'language_id'					=>$this->getLanguageId(),
			  'upload_file_title'			=>$this->getFileTitle(),
			  'upload_file_description'		=>$this->getFileDescription(),
			  'upload_file_rank'			=>$this->getFileRank(),
			  'upload_file_display'			=>$this->getFileDisplay()
			);
			
		return $this->_saved_array;
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function flushFileInfoArray() {
		
  		unset($this->_saved_array);	
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
  	...
  	function filterDisplayableFiles($displayable_file) {
		...
		if ( in_array( $displayable_file , $this->displayable_extension_r ) ) {
			 ...
			 return $this->getFileNameOnServer();
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function checkFileExistence() {
	 	...
	 	try {
	 		...
	 		if ( file_exists( $this->getFileNameOnServer() ) )  {
		
	 			$this->_error_flag = 1;
	 			throw new myExceptions('A file named <em>' . $this->getFileNameAndExtension() . '</em> exists already. Please rename your file.');
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function editedFileInfo() {
		...
		array( 'name' 		=> $this->getNormalizedFileName(),
...
//  ------------------------------------------------------------------------------------------------------------------------------------------
	...
	function printExifInfo() {
		...
		if ( $this->getFileExtension() == '.jpg' ) {
			...
			$exif = exif_read_data( $this->getFileRelativeUrl() , 0 , true );
